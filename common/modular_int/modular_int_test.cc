...
typedef ::testing::Types<ModularInt<uint16_t>, ModularInt<uint32_t>,
                         ModularInt<uint64_t>>
    ModularIntTypes;

// Random uniform distribution for uint64_t.
...
absl::uint128 GenerateRandom(unsigned int* seed) {
  uint64_t hi = GenerateRandom<uint64_t>(seed);
  uint64_t lo = GenerateRandom<uint64_t>(seed);
...
template <typename T>
class ModularIntTest : public ::testing::Test {};
TYPED_TEST_SUITE(ModularIntTest, ModularIntTypes);

TYPED_TEST(ModularIntTest, ModulusTooLarge) {
  ...
  for (Int i = 0; i < kTestingRounds; ++i) {
    ...
    EXPECT_THAT(
        ModularIntParams<Int>::Create(modulus),
...
template <typename T>
std::vector<T> CreateTestModuli() {
  return {};
}

template <>
std::vector<uint16_t> CreateTestModuli() {
  return {::rlwe::kNewhopeModulus};
}

template <>
std::vector<uint32_t> CreateTestModuli() {
  return {::rlwe::kModulus25, ::rlwe::kModulus29};
}

template <>
std::vector<uint64_t> CreateTestModuli() {
  return {::rlwe::kModulus44, (static_cast<uint64_t>(1) << 50) - 1,
          ::rlwe::kModulus59};
}

// Verifies that numbers can be imported and exported properly.
TYPED_TEST(ModularIntTest, ImportExportInt) {
  using Int = typename TypeParam::Int;
  unsigned int seed = 0;

  for (Int modulus : CreateTestModuli<Int>()) {
...
// Verifies that numbers can be imported and exported properly.
TYPED_TEST(ModularIntTest, BatchImportExportInts) {
  using Int = typename TypeParam::Int;
...
TYPED_TEST(ModularIntTest, AddSub) {
  using Int = typename TypeParam::Int;
...
TYPED_TEST(ModularIntTest, InlineAddSub) {
  using Int = typename TypeParam::Int;
...
TYPED_TEST(ModularIntTest, Equality) {
  using Int = typename TypeParam::Int;
...
TYPED_TEST(ModularIntTest, Negate) {
  using Int = typename TypeParam::Int;

  for (Int modulus : CreateTestModuli<Int>()) {
    ASSERT_OK_AND_ASSIGN(auto modulus_params,
                         TypeParam::Params::Create(modulus));
    for (Int i = 0; i < 4 * ::rlwe::kNewhopeModulus; i++) {
...
TYPED_TEST(ModularIntTest, AddRepeatedly) {
  using Int = typename TypeParam::Int;
...
TYPED_TEST(ModularIntTest, Multiply) {
  using Int = typename TypeParam::Int;
...
TYPED_TEST(ModularIntTest, MulInPlace) {
  using Int = typename TypeParam::Int;
  using BigInt = typename internal::BigInt<Int>::value_type;
...
TYPED_TEST(ModularIntTest, MulConstantInPlace) {
  using Int = typename TypeParam::Int;
...
// Verifies that repeated addition works properly.
TYPED_TEST(ModularIntTest, MultiplyRepeatedly) {
  using Int = typename TypeParam::Int;
  using BigInt = typename internal::BigInt<Int>::value_type;
...
TYPED_TEST(ModularIntTest, SmallModulus) {
  using Int = typename TypeParam::Int;
  using BigInt = typename internal::BigInt<Int>::value_type;
  ...
  for (Int modulus : CreateTestModuli<Int>()) {
    ASSERT_OK_AND_ASSIGN(auto modulus_params,
                         TypeParam::Params::Create(modulus));
    const BigInt big_modulus = static_cast<BigInt>(modulus_params->modulus);
    for (int a = 0; a < kExhaustiveTest; a++) {
      ...
      // Equality.
      if (a_BigInt % big_modulus == b_BigInt % big_modulus) {
        EXPECT_TRUE(ma == mb);
        EXPECT_FALSE(ma != mb);
      } else {
        EXPECT_TRUE(ma != mb);
...
TYPED_TEST(ModularIntTest, ModExpModulus) {
  using Int = typename TypeParam::Int;
  using BigInt = typename internal::BigInt<Int>::value_type;
  ...
  for (Int modulus : CreateTestModuli<Int>()) {
    ...
    for (int i = 0; i < kTestingRounds; i++) {
      ...
      for (int exp = 0; exp < kExhaustiveTest; exp++) {
        ...
        expected %= big_modulus;
      }
    }
  }
}

TYPED_TEST(ModularIntTest, InverseModulus) {
  using Int = typename TypeParam::Int;
  ...
  for (Int modulus : CreateTestModuli<Int>()) {
    // Skip non-prime modulus.
    if (modulus == (static_cast<uint64_t>(1) << 50) - 1) {
      continue;
    }
    ASSERT_OK_AND_ASSIGN(auto modulus_params,
                         TypeParam::Params::Create(modulus));
...
TYPED_TEST(ModularIntTest, Serialization) {
  using Int = typename TypeParam::Int;
...
TYPED_TEST(ModularIntTest, ExceedMaxNumCoeffVectorSerialization) {
  using Int = typename TypeParam::Int;
...
TYPED_TEST(ModularIntTest, EmptyVectorSerialization) {
  using Int = typename TypeParam::Int;
...
TYPED_TEST(ModularIntTest, VectorSerialization) {
  using Int = typename TypeParam::Int;
...
TYPED_TEST(ModularIntTest, ExceedMaxNumCoeffVectorDeserialization) {
  using Int = typename TypeParam::Int;
...
TYPED_TEST(ModularIntTest, NegativeVectorDeserialization) {
  using Int = typename TypeParam::Int;
...
TYPED_TEST(ModularIntTest, ImportRandomWithPrngWithSameKeys) {
  using Int = typename TypeParam::Int;
...
TYPED_TEST(ModularIntTest, ImportRandomWithPrngWithDifferentKeys) {
  using Int = typename TypeParam::Int;
...
TYPED_TEST(ModularIntTest, VerifyBarrett) {
  using Int = typename TypeParam::Int;
  ...
  for (Int modulus : CreateTestModuli<Int>()) {
    ...
    for (unsigned int j = 0; j < kTestingRounds; j++) {
      ...
      for (int i = 0; i < kTestingRounds; i++) {
        ...
        EXPECT_EQ(modulus_params->BarrettReduce(a), a % modulus);
      }
    }
  }
}

TYPED_TEST(ModularIntTest, BatchOperations) {
  using Int = typename TypeParam::Int;
...
TYPED_TEST(ModularIntTest, BatchOperationsFailsWithVectorsOfDifferentSize) {
  using Int = typename TypeParam::Int;
...
class FakePrng {
 public:
  absl::StatusOr<uint8_t> Rand8() { return 0; }
  absl::StatusOr<uint64_t> Rand64() { return 0; }
};

TYPED_TEST(ModularIntTest, PrngTemplateParameterizationWorks) {
  using Int = typename TypeParam::Int;
...
TEST(ModularIntTest, ModularIntAvx2Enabled) {
#if defined(__AVX2__) && defined(__FMA__)
  EXPECT_TRUE(IsAvx2EnabledForModularInt());
#else
  EXPECT_FALSE(IsAvx2EnabledForModularInt());
#endif
}
